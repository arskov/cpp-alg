...
public:
    long long totalCost(vector<int> &costs, int k, int candidates)
    {
        ...
        for (int i = 0; i < k; i++)
        {
            if ((pq_right.empty() && !pq_left.empty()) || (!pq_left.empty() && !pq_right.empty() && pq_left.top() <= pq_right.top()))
            {
                total += pq_left.top();
                pq_left.pop();
            ...
            else if ((pq_left.empty() && !pq_right.empty()) || (!pq_left.empty() && !pq_right.empty() && pq_left.top() > pq_right.top()))
            {
                total += pq_right.top();
                pq_right.pop();
                if (left_end < right_begin)
