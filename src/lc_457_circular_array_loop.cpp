...
bool is_valid(vector<int> &nums, int a, int b) {
    if (a == b) return false;
    if ((nums[a] > 0 && nums[b] < 0) || (nums[a] < 0 && nums[b] > 0)) return false;
    return true;
}
    ...
    bool circularArrayLoop(vector<int> &nums) {
        ...
        for (int i = 0; i < arr_size; i++) {
            ...
            while (true) {
                ...
                if (!is_valid(nums, next_slow, slow)) break;
                int next_fast = next(nums, fast);
                if (!is_valid(nums, next_fast, fast)) break;
                fast = next_fast;
                next_fast = next(nums, fast);
                if (!is_valid(nums, next_fast, fast)) break;
                if (next_slow == next_fast) return true;
                slow = next_slow;
                fast = next_fast;
            }
        }
